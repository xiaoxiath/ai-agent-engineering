...
> **为什么需要 Skill 层？——从"万能 Agent"到"专家系统"**
>
> 早期的 Agent 设计倾向于构建一个"万能"Agent，让它通过 Prompt 和工具来应对所有场景。这种方式在 Demo 中表现出色，但在生产环境中迅速暴露出三个问题：（1）Prompt 膨胀——为覆盖所有场景，系统指令越来越长，挤占有效 context 空间；（2）工具选择困难——工具数量超过 20 个后，LLM 的选择准确率显著下降；（3）质量不可控——通用 Prompt 无法针对特定场景做深度优化。Skill 层正是为解决这三个问题而生：通过**意图路由**将用户请求分发到专用 Skill，每个 Skill 拥有精炼的 Prompt 和最小工具集。
...
flowchart TB
    ...
    subgraph 生命周期
        J[Skill 开发] --> K[Skill 测试<br/>Eval Suite]
        K --> L[Skill 发布<br/>版本管理]
        L --> M[Skill 监控<br/>使用率 + 成功率]
    end
```
**图 6.5-1 Skill 知识层架构**——Skill 是介于"工具"和"Agent"之间的抽象层。它将特定领域的 Prompt 模板、工具组合和参数配置封装为可复用的能力单元，是 Agent 工程化的关键基础设施。
