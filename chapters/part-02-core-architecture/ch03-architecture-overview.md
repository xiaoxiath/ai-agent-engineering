...
一个 Agent 在测试环境中表现完美，上线后却频繁"失忆"——它在第 30 轮对话中忘记了用户最初的需求。另一个 Agent 调用了错误的工具，向客户发送了一封措辞不当的邮件。第三个 Agent 在循环推理中消耗了 200 美元的 API 额度却没有产出任何结果。

这些问题的根源不在于 LLM 的能力不足，而在于缺乏系统性的架构设计。就像一栋大楼不能只有混凝土而没有钢筋骨架，一个生产级 Agent 不能只有 LLM 调用而没有状态管理、上下文控制、安全防护和可观测性。

本章提出一个**七层参考架构**作为全书的组织骨架。每一层解决一类核心问题，层与层之间通过明确定义的接口交互。我们还将深入讨论 Agent 控制循环的六种经典模式——从最简单的 ReAct 到最复杂的自适应混合循环——帮助你根据场景选择合适的架构。

```mermaid
graph TB
    subgraph 七层 Agent 参考架构
        L7[编排层 Orchestration] --> L6[评估层 Evaluation]
        L6 --> L5[安全层 Security]
        L5 --> L4[记忆层 Memory]
        L4 --> L3[工具层 Tool]
        L3 --> L2[上下文层 Context]
    ...
    style L3 fill:#3949ab,color:#fff
    style L4 fill:#5c6bc0,color:#fff
    style L5 fill:#e53935,color:#fff
    style L6 fill:#43a047,color:#fff
    style L7 fill:#fb8c00,color:#fff
```

> **设计决策：为什么是七层而非更少？** 一种常见的替代方案是三层架构（核心 + 工具 + 记忆），它实现更简单。但我们在生产实践中发现，安全防护、上下文管理和评估反馈如果不作为独立层，往往会被"塞进"其他层，导致关注点混乱和维护困难。七层架构的代价是接口数量增加和一定的性能开销（层间调用），但它带来的可维护性和可测试性在中大型 Agent 系统中是值得的。对于简单的单工具 Agent，你完全可以合并某些层——关键是理解每层的职责，再决定是否合并。
...
interface ParameterSchema {
  ...
  // ... 省略 158 行，完整实现见 code-examples/ 对应目录
    }
...
interface ReActTrace {
  step: number;
  thought: string;
  // ... 省略 172 行，完整实现见 code-examples/ 对应目录
...
interface ReflectionResult {
  ...
  // ... 省略 167 行，完整实现见 code-examples/ 对应目录
        ].join("\n"),
      },
    ]);
...
type LifecycleState =
  | "created"
  | "initialized"
  // ... 省略 225 行，完整实现见 code-examples/ 对应目录
