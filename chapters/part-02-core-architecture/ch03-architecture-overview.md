...
interface AgentState {
  readonly status: "idle" | "planning" | "executing" | "reflecting" | "completed" | "failed" | "paused";
...
interface AgentError {
  ...
  readonly type: "llm_error" | "tool_error" | "timeout" | "budget_exceeded" | "security_violation";
...
type AgentEvent =
  | { type: "GOAL_SET"; goal: string; conversationId: string; timestamp: number }
  | { type: "PLAN_CREATED"; plan: ExecutionPlan; timestamp: number }
  | { type: "PLAN_UPDATED"; plan: ExecutionPlan; reason: string; timestamp: number }
  | { type: "STEP_STARTED"; stepIndex: number; timestamp: number }
  | { type: "STEP_COMPLETED"; stepIndex: number; result: string; tokenUsage: { prompt: number; completion: number }; durationMs: number; timestamp: number }
  | { type: "STEP_FAILED"; stepIndex: number; error: string; recoverable: boolean; timestamp: number }
  | { type: "TOOL_CALLED"; stepIndex: number; tool: string; args: Record<string, unknown>; timestamp: number }
  | { type: "TOOL_RESULT"; stepIndex: number; tool: string; output: string; isError: boolean; durationMs: number; timestamp: number }
  | { type: "REFLECTION_STARTED"; iteration: number; timestamp: number }
  | { type: "REFLECTION_COMPLETED"; iteration: number; qualityScore: number; refined: boolean; timestamp: number }
  | { type: "AGENT_COMPLETED"; finalAnswer: string; timestamp: number }
  | { type: "AGENT_FAILED"; error: string; timestamp: number }
  | { type: "AGENT_PAUSED"; reason: string; timestamp: number }
  | { type: "AGENT_RESUMED"; timestamp: number };
  ...
  switch (event.type) {
    ...
    case "PLAN_CREATED":
      return {
        ...
        plan: event.plan,
        currentStepIndex: 0,
        context: { ...state.context, lastActivityAt: event.timestamp },
      };
    ...
    case "PLAN_UPDATED":
      return {
        ...
        metrics: {
          ...state.metrics,
          replanCount: state.metrics.replanCount + 1,
        },
        context: { ...state.context, lastActivityAt: event.timestamp },
      };
    ...
    case "STEP_COMPLETED":
      return {
        ...
        metrics: {
          ...state.metrics,
          totalTokens: state.metrics.totalTokens + event.tokenUsage.prompt + event.tokenUsage.completion,
          totalDurationMs: state.metrics.totalDurationMs + event.durationMs,
          llmCallCount: state.metrics.llmCallCount + 1,
        },
        context: { ...state.context, lastActivityAt: event.timestamp },
      };
    ...
    case "TOOL_RESULT":
      return {
        ...
        metrics: {
          ...state.metrics,
          toolErrorCount: state.metrics.toolErrorCount + (event.isError ? 1 : 0),
          totalDurationMs: state.metrics.totalDurationMs + event.durationMs,
        },
        context: { ...state.context, lastActivityAt: event.timestamp },
      };
...
/** 获取当前活跃的计划 */
function getActivePlan(state: AgentState): ExecutionPlan | null {
  return state.plan;
}

/** 获取当前正在执行的步骤 */
function getCurrentStep(state: AgentState): AgentStep | null {
...
/** 获取性能指标 */
function getMetrics(state: AgentState): AgentMetrics {
  return state.metrics;
}

/** 获取未恢复的错误 */
function getUnrecoveredErrors(state: AgentState): readonly AgentError[] {
  return state.errors.filter((e) => !e.recoverable);
}

/** 计算 Token 使用效率 */
function getTokenEfficiency(state: AgentState): number {
  const { totalTokens, llmCallCount } = state.metrics;
  if (llmCallCount === 0) return 0;
  return totalTokens / llmCallCount;
}

/** 计算工具成功率 */
function getToolSuccessRate(state: AgentState): number {
  const { toolCallCount, toolErrorCount } = state.metrics;
  if (toolCallCount === 0) return 1;
  return (toolCallCount - toolErrorCount) / toolCallCount;
}

/** 判断 Agent 是否处于活跃状态 */
function isActive(state: AgentState): boolean {
  return ["planning", "executing", "reflecting"].includes(state.status);
}
...
function getElapsedTime(state: AgentState): number {
  if (!state.context.startedAt) return 0;
  const end = state.status === "completed" || state.status === "failed"
    ? state.context.lastActivityAt
    : Date.now();
  return end - state.context.startedAt;
}
...
function getStateSummary(state: AgentState): {
  ...
  const errors = getUnrecoveredErrors(state);
  return {
    status: state.status,
    ...
    isHealthy: errors.length === 0 && getToolSuccessRate(state) > 0.8,
  };
}
```
...
> **预告**：第四章将深入探讨 **模型集成与 Prompt 工程**——如何与不同的 LLM 提供商集成、如何设计高效的系统 Prompt、以及如何实现 Prompt 版本管理和 A/B 测试。第五章将聚焦 **工具系统的高级话题**——包括 MCP（Model Context Protocol）协议、动态工具发现、工具编排和安全沙箱。
