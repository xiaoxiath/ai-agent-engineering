...
第 3 章的任务不是让读者一次性掌握全部实现细节，而是先建立 Agent 工程的总地图：控制循环、上下文、工具、记忆、安全、评估与编排并不是彼此孤立的主题，而是一个系统的不同侧面。后续主干章节，都会回到这张地图上继续展开。

## 建议接着读

如果你希望沿着本书的主干继续推进，建议下一步阅读 第 5 章《Context Engineering — 上下文工程》。这样可以把本章中的判断框架，继续连接到后续的实现、评估或生产化问题上。
