...
第五章 (工具调用)      ─────────→  工具调用结果进入 L1 工作记忆
第六章 (规划与推理)    ─────────→  L3 任务记忆为规划提供历史参考
第八章 (多 Agent 协作)  ←─────────  共享记忆层实现 Agent 间信息共享
第九章 (评估与测试)    ←─────────  RetrievalEvaluator 为记忆系统提供质量指标
```

> **下一步**：在第八章中，我们将探讨多 Agent 系统如何共享和同步记忆。当多个 Agent 协同工作时，记忆一致性和冲突解决将面临全新的挑战——而本章的 MemoryConflictResolver 和 EntityCentricMemory 将成为解决这些问题的基础。
