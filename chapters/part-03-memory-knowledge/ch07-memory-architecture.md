...
本章构建 Agent 的记忆系统架构，解决 Agent 健忘这一核心问题。记忆是 Agent 从无状态工具进化为有经验的助手的关键能力——没有记忆，Agent 每次对话都是从零开始。本章覆盖工作记忆、短期记忆、长期记忆的三层架构设计，以及记忆的存储、检索、更新和遗忘机制。前置依赖：第 4 章状态管理和第 5 章上下文工程。
