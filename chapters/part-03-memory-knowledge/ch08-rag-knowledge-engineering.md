# 第 8 章 RAG 与知识工程
RAG（Retrieval-Augmented Generation）是当前最被过度简化的 AI 技术之一。很多团队的 RAG 实现就是"把文档切片 → 转为向量 → 存进数据库 → 查询时取 Top-K"，然后困惑于为什么检索质量差、答案不准确、用户不满意。

问题在于，这种"朴素 RAG"忽略了检索管线中的每一个关键细节：文档切片的粒度如何影响语义完整性？嵌入模型的选择如何影响跨语言检索？Top-K 的 K 值过大会引入噪音，过小会遗漏关键信息——最优值取决于查询类型。最重要的是，检索出的文档片段如何与 Agent 的当前上下文融合（而非简单拼接），直接决定了最终生成质量。

本章将 RAG 系统分为三个工程阶段来讨论：**索引阶段**（文档处理、分块策略、嵌入选择）、**检索阶段**（多路召回、重排序、过滤）和**生成阶段**（上下文融合、幻觉控制、引用追踪）。每个阶段的设计决策都会对最终质量产生显著影响。
 ...
 * Embedding 服务接口 — 将文本转换为向量嵌入
  // ... 省略 272 行，完整实现见 code-examples/ 对应目录
...
class GraphBuilder {
  ...
  // ... 省略 368 行，完整实现见 code-examples/ 对应目录
...
interface RAGTestCase {
  ...
  // ... 省略 323 行，完整实现见 code-examples/ 对应目录
    const dotProduct = a.reduce((sum, ai, i) => sum + ai * b[i], 0);
    const magnitudeA = Math.sqrt(a.reduce((sum, ai) => sum + ai * ai, 0));
    const magnitudeB = Math.sqrt(b.reduce((sum, bi) => sum + bi * bi, 0));
    return magnitudeA && magnitudeB ? dotProduct / (magnitudeA * magnitudeB) : 0;
...
interface CacheStore {
  ...
  // ... 省略 97 行，完整实现见 code-examples/ 对应目录
      hash = (hash << 5) - hash + char;
      hash |= 0;
    }
    return hash.toString(16);
...
interface CostConfig {
  ...
  // ... 省略 100 行，完整实现见 code-examples/ 对应目录
