...
在前面的章节中，我们深入探讨了如何构建一个功能强大的单体 Agent——它能理解意图、调用工具、管理记忆、规划任务。然而，随着业务场景的复杂度不断攀升，单一 Agent 不可避免地会面临能力瓶颈。本章将带你走进 **Multi-Agent（多 Agent）** 的世界，学习如何让多个 Agent 协同工作，共同完成复杂任务。
...
flowchart TB
    subgraph 协作模式
        A[中心化<br/>Orchestrator 模式] --> D[适用: 流程确定<br/>强一致性需求]
        B[去中心化<br/>Peer-to-Peer 模式] --> E[适用: 动态任务<br/>弹性扩展需求]
        C[混合式<br/>分层委托模式] --> F[适用: 复杂企业场景<br/>兼顾可控与灵活]
    end
    subgraph 通信机制
        G[同步消息<br/>请求-响应] --> J[延迟敏感型任务]
        H[异步消息<br/>事件驱动] --> K[长时间运行任务]
        I[共享黑板<br/>Blackboard 模式] --> L[需要全局状态共享]
    end
```
**图 9-1 多 Agent 协作模式与通信机制分类**——选择协作模式时，最关键的判断维度不是"哪个更先进"，而是任务的确定性程度：确定性高 → 中心化编排；不确定性高 → 去中心化协商。
...
1. **单一职责**：每个 Agent 只做一件事，且做好这件事。一个"研究员 Agent"不应该同时承担写作任务。
2. **显式通信**：Agent 间的消息必须结构化、可序列化、可审计。避免通过共享可变状态"隐式通信"。
3. **故障隔离**：一个 Agent 的失败不应导致整个系统崩溃。每个 Agent 都应有独立的超时和重试机制。
4. **可替换性**：任何 Agent 都应该可以被另一个实现相同接口的 Agent 替换，不影响系统整体。
5. **渐进复杂**：从最简单的架构开始（单 Agent），只有在明确遇到瓶颈时才引入多 Agent。
6. **人类在环**：关键决策点必须有人工审核机制，特别是涉及不可逆操作（发送邮件、修改数据）时。

这六个原则的优先级排序是：故障隔离 > 单一职责 > 人类在环 > 显式通信 > 可替换性 > 渐进复杂。
...
sequenceDiagram
    ...
    O->>O: 质量检查通过 → 输出
```
**图 9-2 Orchestrator 模式典型工作流**——中心化编排的优势在于每个步骤都可审计、可回溯。但缺点同样明显：Orchestrator 成为单点瓶颈，且任务分解的质量完全取决于它的能力。
