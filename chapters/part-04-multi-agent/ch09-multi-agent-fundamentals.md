...
本章讲解多 Agent 系统的基础理论与核心设计模式。当单个 Agent 的能力边界无法覆盖复杂任务时，多 Agent 协作成为必然选择——但也引入了通信、协调和一致性的新挑战。本章覆盖多 Agent 通信协议、任务分解与分配、共享状态管理和冲突解决机制。前置依赖：第 3–6 章的单 Agent 架构基础。
