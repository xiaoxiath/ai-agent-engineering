...
// ---- 基础类型定义 ----
  // ... 省略 383 行，完整实现见 code-examples/ 对应目录
...
interface CrewTool {
  name: string;
  description: string;
  // ... 省略 234 行，完整实现见 code-examples/ 对应目录
...
interface AgentSDKTool {
  type: 'function';
  name: string;
  // ... 省略 209 行，完整实现见 code-examples/ 对应目录
...
function createStandardMatrix(): DecisionMatrix {
  ...
  // === 技术维度 ===
  // ... 省略 248 行，完整实现见 code-examples/ 对应目录
  console.log('\n===== 最终推荐 =====');
...
interface BuildVsBuyFactors {
  ...
  // ... 省略 131 行，完整实现见 code-examples/ 对应目录
...
│              框架层 (采用)                         │
│  ┌──────┬─────────┬────────┬────────┬────────┬─────────┬────────┬──────────┐│
│  │ ADK  │LangGraph│ CrewAI │AutoGen │ OpenAI │        │ Mastra │Vercel AI ││
│  └──────┴─────────┴────────┴────────┴────────┴─────────┴────────┴──────────┘│
│                                                  │
│              基础设施层 (自建/采用)                │
│  ┌─────────────────────────────────────────────┐ │
...
interface ReviewRequest {
  // ... 省略 369 行，完整实现见 code-examples/ 对应目录
      case 'done':
