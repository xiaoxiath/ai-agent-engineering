...
本章构建 AI Agent 系统的完整威胁模型，系统性地分析 Agent 面临的安全风险。Agent 拥有工具调用权限，意味着安全漏洞的后果会从“信息泄露”升级为“执行破坏性操作”。本章覆盖攻击面分析、威胁分类、风险评估矩阵和防御优先级排序，为后续两章（Prompt 注入防御和信任架构）提供理论基础。前置依赖：第 6 章工具系统。

## 本章你将学到什么

1. 为什么 Agent 安全问题比传统 LLM 应用安全更接近“行为安全”而不仅是“内容安全”
2. 如何从输入、工具、记忆、知识和执行链路去建模攻击面
3. 如何把抽象风险转化为可排序、可优先级化的工程问题
4. 在资源有限时，应该先防哪些高影响风险

## 时效性说明

本章会引用行业框架和编号体系来帮助统一讨论语言，但你更应该记住的是**风险类型、攻击路径和防御优先级**，而不是某一版分类表本身。
...
OWASP Top 10 for Agentic Applications 是用于讨论自主 Agent 系统风险的一套重要框架。与传统 Web 应用安全（OWASP Top 10）和 LLM 应用安全（OWASP LLM Top 10）相比，它更强调 Agent 的自主决策、工具调用和跨系统执行能力所带来的新型风险。对工程团队来说，这类框架最重要的作用，是提供统一的风险语言和优先级视角。
...
如果你希望沿着本书的主干继续推进，建议下一步阅读 第 13 章《Prompt Injection 防御》。这样可以把本章中的判断框架，继续连接到后续的实现、评估或生产化问题上。
