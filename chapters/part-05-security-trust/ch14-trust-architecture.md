...
本章构建 Agent 系统的信任架构——从"默认信任"转向"零信任"的工程化实践。信任架构解决的核心问题是：如何在赋予 Agent 足够能力的同时，确保它不会越权操作。本章覆盖权限模型设计、人机协作审批流、运行时约束执行和信任等级动态调整。前置依赖：第 12 章威胁模型和第 13 章 Prompt 注入防御。
