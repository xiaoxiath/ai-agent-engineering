...
本章构建 Agent 评估的完整体系——从离线基准测试到在线质量监控。Agent 的非确定性特质使得传统软件测试方法不再适用，你需要一套专为 Agent 设计的评估方法论。本章覆盖评估维度定义、自动化评估管线、LLM-as-Judge、人工评估协议和持续质量监控。前置依赖：第 3 章架构总览（理解 Agent 的完整执行流程）。
