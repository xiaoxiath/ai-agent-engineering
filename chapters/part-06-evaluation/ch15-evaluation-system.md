...
## 问题动机：为什么 Agent 评估与传统测试截然不同

发布一个传统 Web 服务时，团队的信心来源很明确——单元测试全绿、集成测试通过、性能基线未退化。但把同样的信心模型套到 Agent 身上，立刻就会发现根基是松动的。

第一层困难是**输出的非确定性**。同一个 Prompt，同一组工具，两次运行可能产生完全不同但都合理的答案。你无法像断言 `assertEqual(expected, actual)` 那样判定对错，因为"对"不再是一个点，而是一个区域。

第二层困难是**行为路径的多样性**。用户说"帮我订明天上午的会议室"，Agent 可能先查日历再搜空闲房间，也可能先搜房间再查日历。两条路径都正确，但传统测试框架只认一条。

第三层困难是**质量的多维性**。Agent 不仅要"做对"，还要"做好"——用最少的 Token、最少的工具调用、在最短时间内完成任务，同时不越权、不泄露信息。这四个维度之间常常存在张力：提高质量可能要多调几次 LLM（降低效率），过于保守的安全策略可能拒绝正常请求（降低完成率）。

传统软件测试解决的是"对不对"的二元问题，而 Agent 评估要回答的是"在多大程度上、以什么代价、是否安全地完成了任务"这个多维连续问题。本章构建的评估体系，正是为了让这个问题有量化的、可追踪的、可自动化的答案。
...
传统软件开发中，先写代码再写测试（或 TDD 中先写测试再写代码）。但 Agent 开发有其独特性：**我们往往不确定 Agent 的最佳行为模式是什么**。Eval-Driven Development（EDD）主张：
...
```typescript
// 成熟度评估器的核心逻辑
// 完整实现见 code-examples/ch15/eval-maturity-assessor.ts
...
```typescript
// 指标归一化与维度评估
// 完整实现见 code-examples/ch15/eval-dimension-framework.ts
...
```typescript
// Mock 系统核心架构
// 完整实现见 code-examples/ch15/tool-mock-system.ts
...
```typescript
// LLM 裁判核心逻辑
// 完整实现见 code-examples/ch15/llm-judge.ts
...
```typescript
// 生产级评测流水线
// 完整实现见 code-examples/ch15/production-eval-pipeline.ts
...
```typescript
// 轨迹评估核心逻辑
// 完整实现见 code-examples/ch15/trajectory-evaluator.ts
