...
本章深入讲解 Agent 基准测试的设计、实施和解读。基准测试是评估 Agent 能力的标准化方法，但设计不当的基准可能产生误导性结论。本章覆盖主流 Agent 基准（SWE-bench、GAIA、WebArena 等）的分析、自定义基准的设计原则、测试执行框架和结果的统计学解读。前置依赖：第 15 章评估体系。
