...
本章构建一套专为 AI Agent 设计的可观测性平台。Agent 系统打破了传统 APM 的三条核心假设——请求路径确定、延迟分布集中、错误类型有限——使得可观测性从"运维辅助"升级为"系统刚需"。本章覆盖追踪、指标、日志三大支柱在 Agent 场景下的重新诠释，以及非确定性路径追踪、Token 成本精确归因和多 Agent 链路上下文传播的完整实现。前置依赖：第 3 章架构总览。

---

## 17.1 从"能跑"到"能看"：Agent 可观测性的必要性

把一个 Agent 系统推上线并不是终点，而是新挑战的起点。一个看起来"成功返回"的请求，可能内部经历了 12 轮无意义循环、消耗了 $2.50 的 Token 成本，最终给出了一个质量堪忧的回答。如果没有完善的可观测性体系，你甚至不知道这件事正在发生。
...
三者通过 **Trace ID** 关联，形成多维度观测体系。典型调查路径：从**指标**发现异常 → 通过**追踪**定位具体请求链路 → 通过**日志**查看每一步的详细推理过程。
