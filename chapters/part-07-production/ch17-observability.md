...
OpenTelemetry 的 GenAI 语义约定（Semantic Conventions for GenAI）已于 2025 年达到 **stable** 状态，为 LLM 调用和 Agent 操作定义了标准化的属性命名。`gen_ai.*` 命名空间下的核心属性——包括 `gen_ai.system`、`gen_ai.request.model`、`gen_ai.usage.input_tokens` / `gen_ai.usage.output_tokens` 等——已被正式纳入 OpenTelemetry 规范并保证向后兼容。
...
| 延迟分解 | TTFT(首字延迟)、生成速度(tokens/s)、总延迟 | 用户体验和容量规划 |
...
**质量优化：** 如果超过 5% 的调用因达到 `max_tokens` 限制而被截断（`finish_reason=length`），Agent 可能收到不完整的响应——增加 `max_tokens` 或优化 Prompt 使 LLM 生成更简洁的回复。
