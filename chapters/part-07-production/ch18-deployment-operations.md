...
本章讲解 Agent 系统的部署与运维工程——从开发环境到生产环境的完整交付流程。Agent 的非确定性行为使得传统的 CI/CD 管线和发布策略需要显著调整。本章覆盖容器化部署、灰度发布、回滚策略、自动扩缩容和生产环境的故障诊断与恢复。前置依赖：第 17 章可观测性工程。

## 本章你将学到什么

1. Agent 系统的部署为什么比传统 Web 服务更复杂
2. 如何理解 Agent 部署中的计算层、编排层、缓存层和持久层
3. 为什么“先有可观测性，再谈运维”对 Agent 尤其重要
4. 小团队与平台团队在部署方案上应如何做不同选择

## 一个部署原则

> 不要一开始就为 Agent 选择“最先进”的部署架构，而应选择“最容易观测、最容易回滚、最容易限制失控后果”的部署架构。

## 本章在主线里的位置

如果第 17 章解决的是“怎么看清系统”，那么第 18 章解决的是“怎样把系统稳定地放到真实环境里运行”。

本章故意放在可观测性之后，是因为部署并不只是把代码发出去，而是把一个带有成本、风险、时延和外部依赖的 Agent 系统置于长期运行状态。没有前一章的可观测性基础，部署章节里的很多策略都会沦为纸面配置。

因此，阅读本章时请始终带着一个工程判断：**部署架构不是为了显得先进，而是为了让系统在真实流量下更容易控制。**
...
AI Agent 系统的部署架构远比传统 Web 服务复杂。但这并不意味着所有团队都需要一开始就采用最重的平台化方案。更现实的做法是先识别：你的系统是否已经具备长任务、重状态、多工具、高延迟方差这些特征；如果还没有，就不必过早复杂化部署形态。一个在浏览器端看似简单的"帮我查一下上周的销售报告"请求，在后端可能触发意图解析、知识库检索、SQL 工具调用、结果格式化等多个异步步骤，每一步都涉及不同的计算特征和资源需求。

一个典型的 Agent 部署拓扑需要考虑以下五个维度。阅读时请把它们理解为“分析维度”，而不是“所有项目都必须具备的五层组件”：
...
**设计决策：为什么很多团队最终会走向 Kubernetes，而不是纯 Serverless？** Agent 工作负载的长连接、有状态和高延迟方差特性，使得 Serverless 的冷启动惩罚和执行时限成为严重瓶颈。Kubernetes 的 Pod 模型允许精确控制资源配比、维持常驻进程避免冷启动、并通过自定义指标实现 Agent 感知的弹性伸缩。Serverless 的适用场景和混合部署策略见 18.9 节。
...
语义缓存是降低 LLM 调用成本的关键技术，通过向量相似度匹配复用历史响应，可减少 30-60% 的重复调用。与传统的精确匹配缓存不同，语义缓存通过 Embedding 向量的余弦相似度实现"近似命中"。完整的语义缓存架构设计（含 LRU 淘汰、TTL 过期、命中率追踪、预热策略）与工程化实现示例详见 **第 19 章 §19.4.2 SemanticCostCache**。
...
部署与运维的重点，不是尽快堆出最复杂的平台能力，而是让 Agent 在真实环境中可控地运行。弹性、扩缩容、发布策略和故障恢复都重要，但它们必须建立在清晰的观测与回滚能力之上。对大多数团队来说，一个更容易理解和控制的部署方案，往往比一个更炫的方案更有工程价值。

## 建议接着读

如果你希望沿着本书的主干继续推进，建议下一步阅读 第 20 章《协议与互操作》。这样可以把本章中的判断框架，继续连接到后续的实现、评估或生产化问题上。
