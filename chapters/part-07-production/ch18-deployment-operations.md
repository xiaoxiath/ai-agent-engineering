...
// 语义缓存实现详见第 19 章 §19.4.2
import {
  AdvancedSemanticCache,
  SemanticCacheConfig,
  EmbeddingProvider,
} from "../ch19-cost-engineering/semantic-cost-cache";
import {
...
语义缓存通过向量相似度匹配实现"近似命中"，相比精确匹配缓存能获得更高的命中率，可在保证质量的前提下显著降低 LLM 调用成本。完整的语义缓存实现（`AdvancedSemanticCache`/`SemanticCostCache`）详见第 19 章 §19.4.2。
