...
## 问题动机：为什么成本工程是 Agent 落地的生死线

2025 年下半年，一家中型 SaaS 公司在内部上线了基于 Claude Opus 的客户成功 Agent。原型阶段，团队对效果赞不绝口——Agent 能准确理解客户问题，查阅知识库，给出专业回复。CTO 拍板全量上线。第一个月的账单到了：$53,000。这个数字超出预算 6 倍，直接触发了 CFO 的成本审查。

这不是个例。几乎所有将 Agent 从原型推向生产的团队都会撞上同一堵墙：**一次看似简单的用户对话，在 Agent 内部可能触发 5 次 LLM 调用、3 次工具执行、2 次因输出格式错误的重试，最终产生的费用是预期的 10 倍。** 而这些隐性成本在 demo 阶段根本看不到——日均 100 次调用和日均 100,000 次调用之间隔着三个数量级的金钱。

问题的根源在于，LLM 的计费模型与传统软件截然不同。传统 API 调用的成本几乎可忽略不计（一次数据库查询的成本在 $0.000001 量级），而一次 LLM 调用的成本在 $0.01–$0.10 量级——高出四到五个数量级。这意味着传统软件工程中"先跑通再优化"的策略在 Agent 领域行不通：你可能还没来得及优化，预算就已经烧完了。

更棘手的是，Agent 系统的成本结构具有 **乘法效应**：token 单价 × 每次调用 token 数 × 每任务调用次数 × 每日任务量 × 重试倍数。任何一个因子的小幅增长，都会在乘法链条上被放大。一个系统提示词从 2,000 token 膨胀到 5,000 token（在迭代中很常见），整个系统的输入成本就增加了 150%。

本章的目标是建立一套系统化的成本工程方法论。我们不会止步于"选便宜的模型"这种粗浅建议，而是从成本建模、不可靠性税分析、智能路由、缓存与压缩、批处理调度、监控告警到预算治理，构建一条完整的成本优化流水线。三个真实世界的案例研究将展示这些技术如何将月度成本从 $50K+ 降至 $8K–$18K 的可持续水平。

我们在第 17 章建立了度量基础设施，在第 18 章讨论了基础设施成本。本章将在此基础上构建完整的成本工程体系。这些优化策略最终需要在第 20 章的多 Agent 协作场景中面对更复杂的成本分摊问题。
...
要优化成本，首先需要精确了解每一分钱花在了哪里。以下是 2026 年主流大语言模型的定价数据（美元/百万 tokens）：
...
一个直观的例子：某 Agent 使用 Claude Sonnet，系统提示词 3,000 tokens（全部缓存命中），用户输入 500 tokens，输出 800 tokens。
...
- **控制输出长度** 比控制输入长度更重要。在 Prompt 中明确要求"简洁回答"或设置 `max_tokens` 上限，往往比压缩输入更有效。
...
即便选择了最具性价比的模型，Prompt 本身仍然是成本的最大来源。一个典型的企业级 Agent 系统中，系统提示词往往长达 2,000–8,000 tokens，而这些提示词在每次请求中都会重复发送。如果 Agent 每天处理 100,000 次请求，仅系统提示词的重复传输就意味着数十亿 tokens 的浪费。
...
| 最小缓存量 | 1,024 tokens | 1,024 tokens | 32,768 tokens |
...
一个真实的教训：某团队在一次"无害"的系统提示词更新中，将一段冗余说明从 200 tokens 扩展到 2,000 tokens（为了"让 Agent 更好地理解上下文"）。由于没有成本监控，这个变更在两周后的月度账单中才被发现——多花了 $12,000。如果有实时的 token 消耗看板和异常告警，这笔损失可以在几小时内被阻止。
...
- `agent_tokens_total{agent, model, direction}` — 累计 token 消耗
...
一个电商平台的客服 Agent 使用 Claude Sonnet，系统提示词包含商品知识库（2,000+ tokens）、退换货政策（1,500+ tokens）、优惠活动（1,000+ tokens）和回答准则（1,500+ tokens），共计约 6,000 tokens。每次对话的用户输入平均仅 200 tokens。
...
| 日均对话量 | 80,000 次（含多轮） |
| 平均输入 tokens | 6,200（系统提示词 6,000 + 用户消息 200） |
| 平均输出 tokens | 300 |
