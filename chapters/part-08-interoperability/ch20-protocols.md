...
本章深入讲解 Agent 互操作性的协议层——MCP（Model Context Protocol）和 A2A（Agent2Agent）。协议标准化是 Agent 生态从"孤岛"走向"互联"的关键基础设施。本章覆盖 MCP 协议的完整规范解析、A2A 协议设计、协议安全性分析和跨框架集成实践。前置依赖：第 6 章工具系统设计。
...
MCP 于 2024 年由 Anthropic 发布，2025 年 12 月捐赠给 Linux Foundation AAIF。关于 MCP 的工具系统集成细节参见第 6 章 6.4 节。
