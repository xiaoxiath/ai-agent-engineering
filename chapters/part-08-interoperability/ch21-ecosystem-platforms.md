    ...
    switch (this.state) {
      ...
      case 'open':
        ...
        if (Date.now() - this.lastFailureTime >= this.config.resetTimeoutMs) {
          ...
          console.log(`[CircuitBreaker] 状态转换: open → half-open`);
          return true;
        }
        return false;
      case 'half-open':
    ...
    switch (this.state) {
      case 'half-open':
        ...
        if (this.successCount >= this.config.halfOpenRequests) {
          ...
          this.failureCount = 0;
          this.successCount = 0;
          console.log(`[CircuitBreaker] 状态转换: half-open → closed`);
        }
        break;
      case 'closed':
        // 连续成功可以重置失败计数
    ...
    switch (this.state) {
      case 'closed':
        if (this.failureCount >= this.config.failureThreshold) {
          this.state = 'open';
          console.log(
            `[CircuitBreaker] 状态转换: closed → open ` +
            `(连续 ${this.failureCount} 次失败)`
          );
        }
        break;
      case 'half-open':
        this.state = 'open';
        console.log(`[CircuitBreaker] 状态转换: half-open → open (半开测试失败)`);
        break;
    }
  }
...
将 Sidecar 模式引入 Agent 系统，`AgentSidecar` 自动注入认证、限流、日志和追踪。`CircuitBreaker`（断路器）防止级联失败。这种基础设施级的关注点分离让 Agent 开发者专注于业务逻辑。
