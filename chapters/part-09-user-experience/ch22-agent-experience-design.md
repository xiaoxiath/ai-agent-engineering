...
你精心设计了 Agent 的架构，优化了它的推理能力，接入了完善的工具链——但上线后用户的反馈却是"不知道它在做什么"、"感觉不受我控制"、"出错了不知道怎么办"。

这是 Agent 产品面临的最大挑战之一：**技术能力和用户体验之间的鸿沟**。传统 UX 设计的核心假设是"用户发起操作，系统执行并返回结果"——这是一个确定性的、用户主导的交互模型。Agent 打破了这个假设：它会自主决策、异步执行多步操作、产出不确定的结果。用户突然从"操作者"变成了"监督者"，而大多数 UI 都没有为这个角色转变做好准备。

本章提出 **Agent Experience（AX）** 的概念，作为 UX 在 Agent 时代的演进。AX 的核心挑战是**信任校准**——帮助用户建立对 Agent 能力的准确预期，既不过度信任（导致灾难性错误），也不过度怀疑（导致体验降级为手动操作）。我们将从信息披露、对话控制、进度透明、信任信号、失败体验和多模态交互六个维度来讨论 AX 设计。

```mermaid
graph TB
    subgraph AX 设计六维度
        D1[渐进式信息披露] --> Trust[信任校准]
        D2[对话控制与中断] --> Trust
        D3[进度与透明度] --> Trust
        D4[信任信号设计] --> Trust
        D5[失败体验设计] --> Trust
    ...
    style Trust fill:#1565c0,color:#fff
```
